...
export function createInteractionBenchmarks(scene) {
  ...
  const step = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x3c4248, roughness: 0.92 }),
  )
  step.name = "BenchmarkStep"
  step.position.set(-0.55, 0.11, 0.18)
  root.add(step)

  const stairs = new THREE.Group()
  stairs.name = "BenchmarkStairs"
  stairs.position.set(-1.55, 0, 0.75)
  root.add(stairs)
  ...
  // Cup is a group so the hand can target the physical handle instead of the
  // cylinder centre. This is the first benchmark for grip-surface semantics.
  const cup = new THREE.Group()
  cup.name = "BenchmarkCup"
  ...
  cupBody.name = "BenchmarkCupBody"
  cup.add(cupBody)
  const cupHandle = new THREE.Mesh(
    new THREE.TorusGeometry(0.045, 0.012, 10, 20, Math.PI * 1.55),
    cupBody.material,
  )
  cupHandle.name = "BenchmarkCupHandle"
  cupHandle.position.set(0.074, 0.005, 0)
  cupHandle.rotation.y = Math.PI / 2
  cup.add(cupHandle)

  const doorPivot = new THREE.Group()
  doorPivot.name = "BenchmarkDoorHinge"
  doorPivot.position.set(0.25, 1.025, -2.15)
  root.add(doorPivot)
  const door = new THREE.Mesh(
    new THREE.BoxGeometry(1.0, 2.05, 0.08),
    new THREE.MeshStandardMaterial({ color: 0x2d2a28, roughness: 0.78 }),
  )
  door.name = "BenchmarkDoor"
  door.position.x = 0.5
  doorPivot.add(door)
  const handle = new THREE.Mesh(
    new THREE.SphereGeometry(0.045, 14, 14),
    new THREE.MeshStandardMaterial({ color: 0xb7b7b7, metalness: 0.55, roughness: 0.3 }),
  )
  handle.name = "BenchmarkDoorHandle"
  handle.position.set(0.86, -0.02, 0.075)
  ...
  bell.name = "BenchmarkDoorbell"
  bell.position.set(1.35, 1.25, -2.08)
  root.add(bell)

  const objectTable = new THREE.Mesh(
    new THREE.BoxGeometry(0.9, 0.08, 0.65),
    new THREE.MeshStandardMaterial({ color: 0x484b50, roughness: 0.85 }),
  )
  objectTable.position.set(0.65, 0.76, 1.5)
  root.add(objectTable)

  const phone = new THREE.Mesh(
    new THREE.BoxGeometry(0.08, 0.15, 0.018),
    new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.4 }),
  )
  phone.name = "BenchmarkPhone"
  phone.position.set(0.85, 0.86, 1.45)
  root.add(phone)
  ...
  magazine.name = "BenchmarkMagazine"
  magazine.position.set(0.45, 0.82, 1.55)
  root.add(magazine)

  const sitApproach = new THREE.Vector3(-2.0, 0, 0.10)
  // Sofa is behind the actor during the actual sit. MotionLab uses lookAt as
  // the facing target before the action, so this target deliberately points
  // away from the sofa, not toward the cushion.
  const sitFacingTarget = new THREE.Vector3(-2.0, 1.0, 1.10)
  ...
  const targets = {
    ...
    OPEN_DOOR: {
      ...
      get lookAt() { return worldPoint(handle) },
      get contactPoint() { return worldPoint(handle) },
      object: door,
      handle,
      doorPivot,
      hand: "right",
    ...
    PICK_UP_CUP: {
      ...
      get lookAt() { return worldPoint(cupHandle) },
      get contactPoint() { return worldPoint(cupHandle) },
      get gripPoint() { return worldPoint(cupHandle) },
      object: cup,
      handle: cupHandle,
      hand: "right",
      massClass: "light",
      type: "handle-grip-light",
    },
    PICK_UP_PHONE: {
      approachPoint: new THREE.Vector3(0.85, 0, 0.92),
      get lookAt() { return worldPoint(phone) },
      get contactPoint() { return worldPoint(phone) },
      get gripPoint() { return worldPoint(phone, new THREE.Vector3(0.03, 0, 0)) },
      object: phone,
      hand: "right",
      secondaryHand: "left",
      massClass: "light",
      type: "thin-object-grip",
    },
    PICK_UP_MAGAZINE: {
      approachPoint: new THREE.Vector3(0.45, 0, 1.0),
      get lookAt() { return worldPoint(magazine) },
      get contactPoint() { return worldPoint(magazine) },
      get gripPoint() { return worldPoint(magazine, new THREE.Vector3(0.09, 0, -0.04)) },
      get secondaryGripPoint() { return worldPoint(magazine, new THREE.Vector3(-0.09, 0, -0.04)) },
      object: magazine,
      hand: "right",
      secondaryHand: "left",
      massClass: "light",
    ...
    SIT_SOFA: {
      approachPoint: sitApproach,
      lookAt: sitFacingTarget,
      seatPoint: new THREE.Vector3(-2.0, 0.49, -0.45),
      // Feet are behind the knees and close to the pelvis/spine vertical axis.
      footLeft: new THREE.Vector3(-2.18, 0.02, -0.37),
      footRight: new THREE.Vector3(-1.82, 0.02, -0.37),
      contactPoint: new THREE.Vector3(-2.0, 0.49, -0.45),
      object: sofa,
      type: "full-body-seat",
    },
    LEAN_WALL: {
      ...
      pelvisContact: new THREE.Vector3(2.03, 0.86, -0.15),
      shoulderContact: new THREE.Vector3(2.03, 1.42, -0.15),
  ...
  return {
    root,
    targets,
    terrain: { step, stairs, stairTreads },
    objects: { wall, sofa, sofaBack, cup, cupBody, cupHandle, phone, magazine, door, handle, doorPivot, bell, step, stairs },
  }
}
